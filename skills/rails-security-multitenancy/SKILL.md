...
description: |
  Apply Rails security and multi-tenant safety practices from 37signals
  (Fizzy's path-based multi-tenant SaaS, Campfire's bot APIs): scoped queries,
  three-layer identity/session/user auth, SSRF defenses, rate limiting, CSRF,
  and tenant-scoped realtime updates. Use when implementing or reviewing auth,
  tenant boundaries, webhooks, file authorization, or any security-sensitive
  endpoint. Tests are written in RSpec.
  Do NOT use for: non-Rails projects, single-tenant apps with no auth or
  external-input surface, pure performance work, or the job layer itself (use
...
- Scope realtime broadcasts and stream names by tenant/account (see rails-hotwire-realtime).
...
- Recurring jobs run outside request context: iterate tenants explicitly (`with_each_tenant`). Serialize `Current.account` into the Sidekiq job arguments and re-establish it in the worker — never rely on ambient `Current` state inside a job (see background-job-designer).
- Test the tenancy middleware in isolation with `Rack::MockRequest`; in RSpec request specs set `default_url_options[:script_name]` from the fixture/factory account.
...
- Verify signatures (e.g. `Stripe::Webhook.construct_event`), then re-fetch canonical state from the source API rather than trusting payload contents/ordering (see rails-webhooks).

## Testing (RSpec)

- Cross-tenant isolation: a request authenticated in account A must 404/403 when reaching for an account-B record. Write the negative case explicitly.
- Scoped lookups: assert wrong/foreign IDs raise `RecordNotFound`, not silent leakage.
- SSRF: stub DNS resolution and assert private/loopback targets are rejected at both create and execute time.
- Rate limiting: drive the endpoint past the threshold and assert the throttled response.
...
- `Current.account`/`Current.user` relied on inside a Sidekiq job without being passed in explicitly.
