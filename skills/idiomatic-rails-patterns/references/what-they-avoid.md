...
> **How to use this file — context, not mandate.** This documents what *37signals*
> avoids and *why*. It is reference for understanding the vanilla-Rails path, not a
> rule the current team must follow. Many teams legitimately use Devise, Pundit,
> service objects, form objects, RSpec, Sidekiq, etc. — that is a settled choice,
> not a smell. **If the project has already adopted one of these, follow the
> project's convention.** Advise the vanilla alternative at most once and in one
> line, then defer to the team and do the work within their stack. Do not push to
> rip out working tools.
