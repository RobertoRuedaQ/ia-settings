...
description: |
  Apply Hotwire, Turbo, Stimulus, and ActionCable best practices for
  real-time Rails interfaces, derived from 37signals (Campfire's cable-heavy
  chat and Fizzy's streams-only kanban). Use when building or reviewing Turbo
  Streams/Frames, Stimulus controllers, broadcast patterns, reconnect/catch-up
  logic, or websocket-driven updates. Tests are written in RSpec + Capybara.
  Do NOT use for: non-Rails projects, plain server-rendered pages with no
  realtime/Turbo component, React/Vue SPAs that own their own client state, or
  pure backend/job work (use background-job-designer for the job side).
---
...
- Scope every stream name by tenant/user: `[board.account, :all_boards]`, `[user, :notifications]` — stream names are isolation boundaries (see rails-security-multitenancy).
...
- Use `broadcast_*_later` variants (backed by your Sidekiq queue) when synchronous broadcasting hurts request latency; keep heavy fan-out work in a worker (see background-job-designer).
...
- Deliver from a Sidekiq worker with persistent HTTP connections, resolving all AR data before posting to threads; invalidate expired subscriptions async.
- Clean up the push subscription client-side on logout.

## Testing (RSpec + Capybara)

- Model broadcasts (turbo-rails channel): assert against the Turbo streams channel.
  ```ruby
  expect { card.update!(title: "New") }
    .to have_broadcasted_to([user, :notifications]).from_channel(Turbo::StreamsChannel)
  ```
  Negative case: `expect { ... }.not_to have_broadcasted_to([user, :notifications])`.
- Custom ActionCable channels: `have_broadcasted_to(room).with(...)` and `have_stream_for` in channel specs (`type: :channel`).
- Controller/request specs: send the turbo-stream Accept header and assert on the rendered stream.
  ```ruby
  post card_path(card), params: { ... }, headers: { "Accept" => "text/vnd.turbo-stream.html" }
  expect(response.media_type).to eq("text/vnd.turbo-stream.html")
  expect(response.body).to have_css(%(turbo-stream[action="replace"][target="#{dom_id(card)}"]))
  ```
- System specs (`type: :system`, JS driver): drive multiple users with `Capybara.using_session("OtherUser") { ... }`, and wait for the subscription before asserting:
  ```ruby
  expect(page).to have_css("turbo-cable-stream-source[connected]")
  ```
