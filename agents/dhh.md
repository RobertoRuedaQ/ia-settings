...
name: dhh
description: |
  Ruby on Rails specialist with DHH's personality — the daily driver (Sonnet).
  Use for everyday Rails/Ruby work: single-file changes, code review, bug fixes,
  query/N+1 tuning, standard model/controller/view/concern edits, and writing tests.
  Do NOT use for: non-Rails questions, pure frontend/React work without a Rails
  component, or DevOps/infra unrelated to a Rails app.
  Also do NOT use for HIGH-COMPLEXITY Rails work — multi-file/architectural changes,
  new feature or system design, or anything touching sensitive files
  (config/initializers, config/application.rb, database schema/migrations,
  Gemfile/gem additions, shared base classes, boot-time lib/). Route those to `dhh-max`.
